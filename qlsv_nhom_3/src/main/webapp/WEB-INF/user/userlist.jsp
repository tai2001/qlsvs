...
<header class="p-3 mb-3 border-bottom">
  ...
  <div class="container-fluid p-5 bg-primary text-white text-center">
  		...
  		<p>Quản Lý Tất Cả Người Dùng Hiện Có!!!</p>
  </div>
  ...
  <div class="container">
  	<div class="row">
  		...
  		<a class="button" href="<%=request.getContextPath() %>/user/form">Thêm Mới</a>
  		</div>
  </div>
  <div class="row">
  	<div class="col-md-12">
  				...
  				<c:if test="${listUser.size() > 0 }">
  					<c:forEach var="user" items="${listUser}">
  						<tr>
  							...
  							<td><c:out value="${user.user_password }"></c:out></td>
  							<td class="text-center">
  							<c:if test="${user.trangthai == true }">
	  								<span class="badge badge-success">Đang hoạt động</span>
  								</c:if> <c:if  test="${user.trangthai == false }">
  									<span class="badge badge-danger">Tạm khóa</span>
  								</c:if></td>
  							<td class="text-center"><a
  								href="<%=request.getContextPath() %>/user/form?id=<c:out value="${user.id }"/>">Chỉnh sửa</a> &nbsp;&nbsp;&nbsp;&nbsp; <a
  								href="<%=request.getContextPath() %>/user/delete?id=<c:out value="${user.id }"/>">Xóa</a></td>
  						</tr>
  					</c:forEach>
  				</c:if>
