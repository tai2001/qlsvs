...
<header class="p-3 mb-3 border-bottom">
    <div class="container">
      <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
        ...
        <ul class="nav col-12 col-lg-auto me-lg-auto mb-2 justify-content-center mb-md-0">
          ...
          <li><a href="#" class="nav-link px-2 disabled px-2 link-secondary"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">Quản Lý Giảng Viên</font></font></a></li>
  ...
  <div class="container-fluid p-5 bg-primary text-white text-center">
  	<c:if test="${lecturer != null }">
  		<h1>CẬP NHẬT DANH MỤC GIẢNG VIÊN</h1>
  	</c:if>
  	<c:if test="${lecturer == null }">
  		<h1>THÊM MỚI DANH MỤC GIẢNG VIÊN</h1>
  	</c:if>
  ...
  <div class="container">
  		<div class="row">
  			<div class="col-md-6 offset-md-3" style="margin-top: 20px">
  				<c:if test="${lecturer != null }">
  					<form action="<%=request.getContextPath()%>/lecturer" methhod="post">
  				</c:if>
  				<c:if test="${lecturer == null }">
  					<form action="<%=request.getContextPath()%>/lecturer" methhod="post">
  				</c:if>
  				<c:if test="${lecturer != null }">
  					<div class="form-group">
  						...
  						readonly placeholder="Nhập tên id" name="id"
  				...
  				<div class="form-group">
  					<label>Họ Tên</label> <input type="text" class="form-control"
  						...
  						value"<c:out value="${lecturer.hoten }"/>">
  				</div>
  				<div class="form-group">
  					<label>Giới tính</label> <input type="text" class="form-control"
  						placeholder="Nhập giới tính" name="gioitinh"
  						value"<c:out value="${lecturer.gioitinh }"/>">
  				</div>
  				<div class="form-group">
  					<label>Ngày sinh</label> <input type="text" class="form-control"
  						placeholder="Nhập ngày sinh" name="ngaysinh"
  						value"<c:out value="${lecturer.ngaysinh }"/>">
  				</div>
  				<div class="form-group">
  					<label>Địa chỉ</label> <input type="text" class="form-control"
  						placeholder="Nhập địa chỉ" name="diachi"
  						value"<c:out value="${lecturer.diachi }"/>">
  				</div>
  				<div class="form-group">
  					<label>Ảnh đại diện</label> <input type="text" class="form-control"
  						placeholder="Nhập ảnh đại diện" name="anhdaidien"
  						value"<c:out value="${lecturer.anhdaidien }"/>">
  				</div>
  				<div class="form-group">
  					<label>Chuyên môn</label> <input type="text" class="form-control"
  						placeholder="Nhập chuyên môn" name="chuyenmon"
  						value"<c:out value="${lecturer.chuyenmon }"/>">
  				</div>
  				<div class="form-group">
  					<label>Bằng cấp</label> <input type="text" class="form-control"
  						placeholder="Nhập bằng cấp" name="bangcap"
  						value"<c:out value="${lecturer.bangcap }"/>">
  				</div>
  				<div class="form-group">
  					<label>Sđt</label> <input type="text" class="form-control"
  						placeholder="Nhập sđt" name="sdt"
  						value"<c:out value="${lecturer.sdt }"/>">
  				</div>
  				<div class="form-group">
  					<label>Email</label> <input type="text" class="form-control"
  						placeholder="Nhập email" name="email"
  						value"<c:out value="${lecturer.email }"/>">
  				</div>
  				...
  				<div class="form-group">
  					<label>Trạng thái</label> <select class="form-control"
  						name="trangthai">
  						<option value="true"
  							selected<c:if test="${lecturer.trangthai == true ? true : false}"></c:if>>
  							Đang hoạt động</option>
  						<option value="false"
  							selected<c:if test="${lecturer.trangthai == false ? true : false}"></c:if>>
  							Tạm khóa</option>
  						</select>
  				</div>
  				<c:if test="${lecturer != null }">
