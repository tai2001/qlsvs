...
<header class="p-3 mb-3 border-bottom">
  ...
  <div class="container-fluid p-5 bg-primary text-white text-center">
  		...
  		<p>Quản Lý Tất Cả Giảng Viên Hiện Có!!!</p>
  </div>
  ...
  <div class="container">
  	<div class="row">
  		...
  		<a class="button" href="<%=request.getContextPath() %>/lecturer/form">Thêm Mới</a>
  		</div>
  </div>
  <div class="row">
  	<div class="col-md-12">
  		<table class="table table-bordered">
  			<thead class="thead-light">
  				...
  				<c:if test="${listLecturer.size() > 0 }">
  					<c:forEach var="lecturer" items="${listLecturer}">
  						<tr>
  							<td class="text-center"><c:out value="${lecturer.id }"/></td> 
  							<td><c:out value="${lecturer.hoten }"></c:out></td>
  							<td><c:out value="${lecturer.gioitinh }"></c:out></td>
  							<td><c:out value="${lecturer.ngaysinh }"></c:out></td>
  							<td><c:out value="${lecturer.diachi }"></c:out></td>
  							<td><c:out value="${lecturer.anhdaidien }"></c:out></td>
  							<td><c:out value="${lecturer.chuyenmon }"></c:out></td>
  							<td><c:out value="${lecturer.bangcap }"></c:out></td>
  							<td><c:out value="${lecturer.sdt }"></c:out></td>
  							<td><c:out value="${lecturer.email }"></c:out></td>
  							<td class="text-center">
  							<c:if test="${lecturer.trangthai == true }">
	  								<span class="badge badge-success">Đang hoạt động</span>
  								</c:if> <c:if  test="${lecturer.trangthai == false }">
  									<span class="badge badge-danger">Tạm khóa</span>
  								</c:if></td>
  							<td class="text-center"><a
  								href="<%=request.getContextPath() %>/lecturer/form?id=<c:out value="${lecturer.id }"/>">Chỉnh sửa</a> &nbsp;&nbsp;&nbsp;&nbsp; <a
  								href="<%=request.getContextPath() %>/lecturer/delete?id=<c:out value="${lecturer.id }"/>">Xóa</a></td>
  						</tr>
  					</c:forEach>
  				</c:if>
