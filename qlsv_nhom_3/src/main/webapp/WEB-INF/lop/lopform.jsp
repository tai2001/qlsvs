...
<header class="p-3 mb-3 border-bottom">
  ...
  <div class="container-fluid p-5 bg-primary text-white text-center">
  	<c:if test="${lop != null }">
  		<h1>CẬP NHẬT DANH MỤC LỚP</h1>
  	</c:if>
  	<c:if test="${lop == null }">
  		<h1>THÊM MỚI DANH MỤC LỚP</h1>
  	</c:if>
  ...
  <div class="container">
  		<div class="row">
  			<div class="col-md-6 offset-md-3" style="margin-top: 20px">
  				<c:if test="${lop != null }">
  					<form action="<%=request.getContextPath()%>/lop" methhod="post">
  				</c:if>
  				<c:if test="${lop == null }">
  				<form action="<%=request.getContextPath()%>/lop" methhod="post">
  				</c:if>
  				<c:if test="${lop != null }">
  					<div class="form-group">
  						...
  						value="<c:out value="${lop.id }"/>">
  					</div>
  				</c:if>
  				<div class="form-group">
  					<label>Mã lớp</label> <input type="text" class="form-control"
  						placeholder="Nhập mã lớp" name="malop"
  						value"<c:out value="${lop.malop }"/>">
  				</div>
  				<div class="form-group">
  					<label>Họ Tên Giảng Viên Chủ Nhiệm</label> <input type="text" class="form-control"
  						placeholder="Nhập họ tên giảng viên chủ nhiệm" name="hotengvcn"
  						value"<c:out value="${lop.hotengvcn }"/>">
  				</div>
  				<div class="form-group">
  					<label>Trạng thái</label> <select class="form-control"
  						name="trangthai">
  						<option value="true"
  							selected<c:if test="${lop.trangthai == true ? true : false}"></c:if>>
  							Đang hoạt động</option>
  						<option value="false"
  							selected<c:if test="${lop.trangthai == false ? true : false}"></c:if>>
  							Tạm khóa</option>
  						</select>
  				</div>
  				<c:if test="${lop != null }">
