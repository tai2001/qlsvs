...
<header class="p-3 mb-3 border-bottom">
  ...
  <div class="container-fluid p-5 bg-primary text-white text-center">
  	<c:if test="${khoa != null }">
  		<h1>CẬP NHẬT DANH MỤC KHOA</h1>
  	</c:if>
  	<c:if test="${khoa == null }">
  		<h1>THÊM MỚI DANH MỤC KHOA</h1>
  	</c:if>
  ...
  <div class="container">
  		<div class="row">
  			<div class="col-md-6 offset-md-3" style="margin-top: 20px">
  				<c:if test="${khoa != null }">
  					<form action="<%=request.getContextPath()%>/khoa" methhod="post">
  				</c:if>
  				<c:if test="${khoa == null }">
  				<form action="<%=request.getContextPath()%>/khoa" methhod="post">
  				</c:if>
  				<c:if test="${khoa != null }">
  					<div class="form-group">
  						...
  						value="<c:out value="${khoa.id }"/>">
  					</div>
  				</c:if>
  				<div class="form-group">
  					<label>Tên khoa</label> <input type="text" class="form-control"
  						placeholder="Nhập tên khoa" name="tenkhoa"
  						value"<c:out value="${khoa.tenkhoa }"/>">
  				</div>
  				<div class="form-group">
  					<label>Trạng thái</label> <select class="form-control"
  						name="trangthai">
  						<option value="true"
  							selected<c:if test="${khoa.trangthai == true ? true : false}"></c:if>>
  							Đang hoạt động</option>
  						<option value="false"
  							selected<c:if test="${khoa.trangthai == false ? true : false}"></c:if>>
  							Tạm khóa</option>
  						</select>
  				</div>
  				<c:if test="${khoa != null }">
